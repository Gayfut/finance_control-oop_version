...
class Sorter:
    ...
    def sort_articles(self, sort_type_name, sort_category_name, sort_date_start=None, sort_date_end=None):
        articles_list = self.__articles_manager.get_articles()
        sorted_article_list = []

        for _article in articles_list:
            if (sort_type_name in (settings.ARTICLE_INCOME, settings.ARTICLE_OUTLAY) and _article.article_type != sort_type_name):
                continue
